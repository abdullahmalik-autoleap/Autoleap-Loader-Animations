interface AutoleapWordmarkProps extends React.SVGProps<SVGSVGElement> {
  letterClassName?: string;
  fillValue?: string;
}

export function AutoleapWordmark({
  letterClassName,
  fillValue,
  ...svgProps
}: AutoleapWordmarkProps) {
  const darkFill = fillValue ?? "#3D4242";
  const greenFill = fillValue ?? "#00C19D";

  // Tag dark vs green paths with theme-part classes so the dark-stage CSS
  // in globals.css can reliably swap "Auto" to a light grey without touching
  // the green "Leap" glyphs.
  const darkPartClass = ["al-wm-dark-part", letterClassName].filter(Boolean).join(" ");
  const greenPartClass = ["al-wm-green-part", letterClassName].filter(Boolean).join(" ");

  return (
    <svg viewBox="0 0 627 124" xmlns="http://www.w3.org/2000/svg" {...svgProps}>
      <path
        data-letter="A"
        className={darkPartClass}
        fill={darkFill}
