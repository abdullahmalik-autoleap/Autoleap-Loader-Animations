...
export function StrokeReveal({ replayKey }: AnimationProps) {
  const scope = useRef<SVGSVGElement>(null);

  useGSAP(
    () => {
      const elements = Array.from(
        scope.current?.querySelectorAll<SVGGeometryElement>(".al-stroke-letter") ?? [],
      );
      ...
      gsap.set(elements, {
        strokeDasharray: (i) => `${lengths[i]} ${lengths[i]}`,
      });
  ...
  return (
    ...
    >
      {letters.map((letter) => {
        const partClass = letter.theme === "dark" ? "al-wm-dark-part" : "al-wm-green-part";
        const className = `al-stroke-letter ${partClass}`;
        return letter.tag === "path" ? (
          <path
            key={letter.key}
            className={className}
            d={letter.d}
